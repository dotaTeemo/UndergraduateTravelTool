...
import com.wego.userActivity.userTravel.RVAdapter;
...
public class SCRecyclerView extends RecyclerView {
   ...
   private boolean isStartScroll;
    ...
    public SCRecyclerView(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public SCRecyclerView(Context context, @Nullable AttributeSet attrs, int defStyle) {
        ...
        switch (e.getAction()) {
            case MotionEvent.ACTION_DOWN:
                if (mMenuState == MENU_CLOSED) {
                    ...
                    //获得这个view的ViewHolder
                    SCAdapter.Holder holder = (SCAdapter.Holder) getChildViewHolder(view);
                    //获得这个view的position
                    mPosition = holder.getAdapterPosition();
                    //获得这个view的整个布局
                    mItemLayout = holder.SCllLayout;
                    //获得这个view的删除按钮
                    tvDelete = holder.SCDelete;
                    //这个view的整个置顶按钮
                    //tvTop = holder.SCTop;
                    //两个按钮的宽度
